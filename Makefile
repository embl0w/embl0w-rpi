...
setup:
	$(SUDO) apt update
	$(SUDO) apt install -y --no-install-recommends sudo build-essential ca-certificates binfmt-support qemu-user-static debootstrap squashfs-tools cpio mtools curl git gnupg time
	mkdir -p $(BUILDDIR)
